...
export function isCorrect(question: Question, answer: string): boolean {
    const answerGrammar = answer.trim().toLowerCase();
    const expectedAnswer = question.expected.trim().toLowerCase();
    if (answerGrammar === expectedAnswer) {
        return true;
    }
    return false;
}
...
export function toShortForm(question: Question): string {
    const gettingID = question.id;
    const nameTen = question.name.substring(0, 10);
    return gettingID + ":" + " " + nameTen;
}
...
export function toMarkdown(question: Question): string {
    ...
    let combined = nameLine + bodyLine;
...
export function publishQuestion(question: Question): Question {
    const newVersion = { ...question, published: !question.published };
    return newVersion;
}
...
export function addOption(question: Question, newOption: string): Question {
    const newV = { ...question, options: [...question.options, newOption] };
    return newV;
}
